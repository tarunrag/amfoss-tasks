...
impl Coordinator {
    ...
    pub fn initialize(&self) -> Result<()> {
        ...
        if !self.data_dir.exists() {
            info!("data directory not found, creating {}", self.data_dir.display());
            fs::create_dir_all(&self.data_dir)
                .with_context(|| format!("failed to create data dir"))?;
        }

        if !self.cache_dir.exists() {
            info!("cache directory not found, creating {}", self.cache_dir.display());
            fs::create_dir_all(&self.cache_dir)
                .with_context(|| format!("failed to create cache dir"))?;
        }
